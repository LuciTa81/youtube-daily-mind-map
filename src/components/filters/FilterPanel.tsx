...
export function FilterPanel({
  ...
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              검색 결과 {hasSearch ? `${searchResultCount}개` : "없음"}
            </p>
          </div>
          {(searchQuery || categoryFilter || channelQuery || lowConfidenceOnly) ? (
            <button
              type="button"
              className="rounded-md border border-slate-200 px-2 py-1 text-[11px] font-semibold text-slate-500 hover:bg-slate-50 hover:text-slate-800"
              onClick={() => {
                onSearchQueryChange("");
                onCategoryFilterChange("");
                onChannelQueryChange("");
                onLowConfidenceOnlyChange(false);
              }}
            >
              초기화
            </button>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div>
          <h2 className="text-sm font-semibold text-slate-900">마인드맵 표시 설정</h2>
